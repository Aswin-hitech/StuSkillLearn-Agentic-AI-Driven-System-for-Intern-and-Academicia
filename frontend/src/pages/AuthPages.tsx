...
const registerRoles: Array<{ role: Exclude<UserRole, 'ADMIN'>; label: string }> = [
  { role: 'STUDENT', label: 'Student' },
  { role: 'COMPANY', label: 'Industry' },
  { role: 'ACADEMICIAN', label: 'Academician' },
...
function AuthShell({ children }: { children: React.ReactNode }) {
  return <div className="min-h-screen bg-[#f6f8fb] px-4 py-8 sm:py-14"><div className="mx-auto max-w-md"><Link to="/" className="mb-6 inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:underline transition-colors duration-300"><ArrowLeft className="h-4 w-4" /> Back to home</Link><div className="surface p-6 shadow-soft sm:p-8"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-slate-950 text-sky-400"><Sparkles className="h-5 w-5" /></div><p className="font-semibold text-slate-950">StuSkillLink</p></div>{children}</div></div></div>
}
...
export function LoginPage() {
  ...
  return <AuthShell><div className="mt-7"><h1 className="text-2xl font-semibold tracking-tight text-slate-950">Sign in to your workspace</h1><p className="mt-2 text-sm leading-6 text-slate-500">Use your StuSkillLink account credentials.</p></div>{error ? <Notice tone="error">{error}</Notice> : null}<form className="mt-5 space-y-4" onSubmit={submit}><label><span className="label">Email</span><input className="input" value={email} onChange={(e) => setEmail(e.target.value)} type="email" /></label><label><div className="mb-1.5 flex items-center justify-between"><span className="label mb-0">Password</span><Link className="text-xs font-semibold text-blue-600 hover:underline" to="/forgot-password">Forgot password?</Link></div><input className="input" value={password} onChange={(e) => setPassword(e.target.value)} type="password" /></label><button className="btn-primary w-full" disabled={busy} type="submit">{busy ? 'Signing in…' : 'Sign in'}</button></form><p className="mt-6 text-center text-sm text-slate-500">Need an account? <Link className="font-semibold text-blue-600 hover:underline" to="/register/STUDENT">Register</Link></p></AuthShell>;
}
...
export function RegisterPage() {
  ...
  return <AuthShell><div className="mt-7 flex items-start gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-sky-50 text-sky-700"><Icon className="h-5 w-5" /></div><div><h1 className="text-2xl font-semibold tracking-tight text-slate-950">Create {normalized.toLowerCase()} account</h1><p className="mt-1 text-sm text-slate-500">You can complete the role profile after registration.</p></div></div><div className="mt-5 flex flex-wrap gap-2">{registerRoles.map((item) => <Link key={item.role} className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold transition-all duration-200 ${item.role === normalized ? 'bg-slate-950 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`} to={`/register/${item.role}`}>{item.label}</Link>)}</div>{error ? <div className="mt-5"><Notice tone="error">{error}</Notice></div> : null}<form className="mt-6 space-y-4" onSubmit={submit}><label><span className="label">Email</span><input className="input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></label><label><span className="label">Password</span><input className="input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={12} required /><span className="mt-1 block text-xs text-slate-400">At least 12 characters with uppercase, lowercase and a number.</span></label><button className="btn-primary w-full" disabled={busy} type="submit">{busy ? 'Creating account…' : 'Create account'}</button></form></AuthShell>;
}
