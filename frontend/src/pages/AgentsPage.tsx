...
const definitions = [
  ['Intake & Eligibility Agent', 'Validates profile readiness, policy gates and deterministic eligibility inputs.'],
  ['Skill & Communication Intelligence Agent', 'Extracts skills, maps gaps, evaluates LSRW and generates learning actions.'],
  ['Portfolio & Preference Agent', 'Maintains credential evidence and student preference state.'],
  ['Matching & Allocation Agent', 'Explains matches while deterministic services own ranking, constraints and allocation.'],
  ['Academician Engagement Agent', 'Matches FDP, research and consultancy opportunities to academician expertise.'],
  ['Monitoring, Notification & Escalation Agent', 'Tracks offers, feedback and keeps vacant seats moving through ranked candidates.'],
  ['Curriculum Framing Agent', 'Turns institution skill-gap and hiring-outcome evidence into auditable curriculum recommendations.'],
];
...
export function AgentsPage() {
  ...
  return <>
    <PageHeader
      ...
      title="Seven audited agents, deterministic decisions"
    ...
    <div className="mb-5"><Notice tone={langGraphActive ? 'success' : 'info'}>
      {langGraphActive ? 'LangGraph orchestration verified · seven checkpointed nodes completed.' : 'Run the seven-agent cycle to create the first LangGraph checkpoint.'}
    </Notice></div>
