...
type CompanyDashboard={
  profile:{company_name:string;industry:string;website:string;description:string;recruiter_name:string;office_locations:string[];verified:boolean;verification_status:string};
...
export function CompanyDashboardPage(){
  ...
  {!data.profile.verified?<div className="mb-5"><Notice>Your company account is <strong>{data.profile.verification_status}</strong>. You can complete your profile, but opportunity publishing, ranking and allocation controls stay locked until Admin verification.</Notice></div>:null}<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Opportunities" value={data.metrics.opportunities}/><MetricCard label="Applicants" value={data.metrics.applicants}/><MetricCard label="Active allocations" value={data.metrics.active_allocations}/><MetricCard label="Learning programs" value={data.metrics.learning_programs}/></div>
...
export function CompanyAllocationsPage(){
  ...
  async function lifecycle(id:number,status:'IN_PROGRESS'|'COMPLETED'){
    await api(`/sih/company/allocations/${id}/lifecycle`,{method:'PATCH',token,body:jsonBody({status})});
    setMessage(status==='IN_PROGRESS'?'Internship marked in progress.':'Engagement marked completed; verified company feedback is now enabled.');
    await load();
  }
    ...
    <div className="space-y-4">
      {items.length?items.map(x=><div className="surface p-5 sm:p-6" key={x.id}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">{x.status==='ACCEPTED'?<button className="btn-secondary" onClick={()=>void lifecycle(x.id,'IN_PROGRESS')} type="button"><RefreshCw className="h-4 w-4"/>Start engagement</button>:null}{x.status==='IN_PROGRESS'?<button className="btn-secondary" onClick={()=>void lifecycle(x.id,'COMPLETED')} type="button"><CheckCircle2 className="h-4 w-4"/>Mark completed</button>:null}<button className="btn-secondary" disabled={x.status!=='COMPLETED'} onClick={()=>setActiveId(activeId===x.id?null:x.id)} type="button"><CheckCircle2 className="h-4 w-4"/>{x.status==='COMPLETED'?(activeId===x.id?'Close feedback':'Add feedback'):'Feedback after completion'}</button></div>
        </div>
