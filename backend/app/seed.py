...
def seed_demo_data(db: Session) -> None:
    ...
    db.add_all([
        DigitalBadge(student_profile_id=student.id, title="Industry API Foundations", issuer="TechNova Labs", verified=True, credential_url="https://example.com/badge/api"),
        DigitalBadge(student_profile_id=student.id, title="Python Data Practice", issuer="NPTEL", verified=True, credential_url="https://nptel.ac.in/"),
    ])
    ...
    for email, name, cgpa, category, skills, lsrw in extra_students:
        ...
        db.add(profile)
        db.flush()
        db.add(LSRWAssessment(student_profile_id=profile.id, listening=lsrw, speaking=lsrw, reading=lsrw + 2, writing=lsrw - 2, overall=lsrw, details={"method": "demo baseline"}))
    ...
    for profile in extras:
        profile.preferences = {**(profile.preferences or {}), "ranked_opportunity_ids": [opportunities[0].id, opportunities[2].id], "ranked_company_ids": [company.id, second_company.id]}
    ...
    db.add_all([
        AcademicianOpportunity(opportunity_type="FDP", title="Applied GenAI Faculty Development Program", host_name="TechNova Labs", description="Five-day industry-led FDP on production GenAI systems.", required_expertise=["Machine Learning", "NLP"], location="Chennai", mode="HYBRID", deadline="2026-10-05"),
        AcademicianOpportunity(opportunity_type="RESEARCH", title="Responsible AI Joint Research Call", host_name="DataWorks Systems", description="Joint industry-academia research on explainable ranking and model governance.", required_expertise=["Machine Learning", "Data Science"], location="Remote", mode="REMOTE", deadline="2026-10-18"),
        AcademicianOpportunity(opportunity_type="CONSULTANCY", title="Curriculum-to-Industry Skill Mapping Consultancy", host_name="TechNova Labs", description="Short consultancy to align course outcomes with live role requirements.", required_expertise=["Data Science", "Python"], location="Bengaluru", mode="HYBRID", deadline="2026-10-25"),
    ])

    db.commit()

    # Complete the demo ranking/allocation state after IDs exist.
    from app.services import allocate_opportunity, ensure_application, refresh_curriculum_insights, refresh_student_matches, refresh_learning_recommendations
