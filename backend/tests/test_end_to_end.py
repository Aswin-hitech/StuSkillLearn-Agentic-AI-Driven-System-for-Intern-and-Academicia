...
def test_only_four_product_roles_can_register():
    reset_db()
    with TestClient(app) as client:
        response = client.post('/api/v1/auth/register/ADMIN', json={'email': 'gov@example.com', 'password': 'Password123'})
        assert response.status_code == 400
        for role in ['STUDENT', 'COMPANY', 'ACADEMICIAN', 'INSTITUTION']:
            response = client.post(f'/api/v1/auth/register/{role}', json={'email': f'{role.lower()}2@example.com', 'password': 'Password123'})
