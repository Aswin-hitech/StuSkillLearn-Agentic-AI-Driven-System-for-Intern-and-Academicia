...
def test_proposal_feature_coverage_smoke():
    ...
    with TestClient(app) as client:
        ...
        provider = client.get('/api/v1/sih/system/provider').json()
        ...
        integrations = client.get('/api/v1/sih/system/integrations').json()
        ...
        badge_id = portfolio['badges'][0]['id']
        verify = client.get(f'/api/v1/sih/badges/{badge_id}/verify')
        ...
        # Automated offer letter + real company feedback loop.
        ...
        # Six agents and automatic reallocation.
        ...
        offered = next(x for x in client.get('/api/v1/sih/company/allocations', headers=company).json() if x['student_name'] == 'Arjun Kumar' and x['status'] == 'OFFERED')
        reject = client.post(f"/api/v1/sih/student/offers/{offered['id']}/respond", headers=student, json={'response': 'REJECTED'})
        assert reject.status_code == 200
        after = client.get('/api/v1/sih/company/allocations', headers=company).json()
        assert any(x['round'] >= 2 and x['status'] == 'OFFERED' for x in after)
        ...
        # Product has only the four roles from the PPT.
        blocked = client.post('/api/v1/auth/register/GOVERNMENT', json={'email': 'gov@example.com', 'password': 'Password123'})
